...
export const useStrategyStore = create<StrategyState>((set, get) => ({
  ...
  fetchStrategies: async () => {
    ...
    try {
      const res = await gatewayRpc<{ strategies?: StrategyDefinition[] }>("strategies.list", {});
      if (res.ok) {
        set({ strategies: res.payload?.strategies ?? [] });
      }
    } catch (err) {
  ...
  fetchExecutions: async (strategyId?: string) => {
    try {
      const res = await gatewayRpc<{ executions?: StrategyExecution[] }>("strategies.executions", {
        strategyId,
      });
      if (res.ok) {
        set({ executions: res.payload?.executions ?? [] });
      }
    } catch {
      // silent
    }
