...
export const useStrategyStore = create<StrategyState>((set, get) => ({
  ...
  fetchStrategies: async () => {
    ...
    try {
      ...
      if (res.ok) {
        set({
          strategies:
            ((res.payload as Record<string, unknown>).strategies as StrategyDefinition[]) ?? [],
        });
      }
    } catch (err) {
  ...
  fetchExecutions: async (strategyId?: string) => {
    try {
      ...
      if (res.ok) {
        set({
          executions:
            ((res.payload as Record<string, unknown>).executions as StrategyExecution[]) ?? [],
        });
      }
    } catch {
      // silent
  ...
  runBacktest: async (strategyId, config) => {
    ...
    try {
      ...
      if (res.ok) {
        set({ backtestResult: res.payload as unknown as BacktestResult });
      } else {
