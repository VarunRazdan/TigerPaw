...
export function useClosePosition(): {
  ...
  const close = useCallback(async (params: ClosePositionParams): Promise<OrderStatus> => {
    // Validate position exists and quantity is within bounds
    const positions = useTradingStore.getState().positions;
    const position = positions.find(
      (p) => p.extensionId === params.extensionId && p.symbol === params.symbol,
    );

    if (!position) {
      const s: OrderStatus = {
        status: "error",
        message: `No open position found for ${params.symbol} on ${params.extensionId}`,
      };
      setState(s);
      return s;
    }

    if (params.quantity !== undefined && params.quantity > position.quantity) {
      const s: OrderStatus = {
        status: "error",
        message: `Close quantity (${params.quantity}) exceeds position size (${position.quantity})`,
      };
      setState(s);
      return s;
    }
