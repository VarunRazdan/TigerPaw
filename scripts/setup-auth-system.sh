...
# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/tigerpaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/tigerpaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/tigerpaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now tigerpaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status tigerpaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
