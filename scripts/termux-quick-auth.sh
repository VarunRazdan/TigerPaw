...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/tigerpaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Tigerpaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/tigerpaw/scripts/mobile-reauth.sh'" --id tigerpaw-auth
        ;;
    *)
