...
BASE_IMAGE="${BASE_IMAGE:-tigerpaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-tigerpaw-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=tigerpaw.sandbox=1)
NOTE
