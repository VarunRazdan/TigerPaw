...
const logRunner = (message, deps) => {
  ...
  deps.stderr.write(`[tigerpaw] ${message}\n`);
};

const runTigerpaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["tigerclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
