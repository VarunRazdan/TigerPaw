...
DMG_TEMP="$(mktemp -d /tmp/tigerpaw-dmg.XXXXXX)"
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/tigerpaw-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/tigerpaw-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/tigerpaw-dmg-limits.txt
