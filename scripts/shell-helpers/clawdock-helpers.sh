#!/usr/bin/env bash
# ClawDock - Docker helpers for Tigerpaw
# Inspired by Simon Willison's "Running Tigerpaw in Docker"
# https://til.simonwillison.net/llms/tigerpaw-docker
#
# Installation:
...
# Common paths to check for Tigerpaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/tigerpaw"
  "${HOME}/workspace/tigerpaw"
  "${HOME}/projects/tigerpaw"
  "${HOME}/dev/tigerpaw"
  "${HOME}/code/tigerpaw"
  "${HOME}/src/tigerpaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "Found Tigerpaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/tigerpaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "Tigerpaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/varunrazdan/tigerpaw.git ~/tigerpaw"
    echo "  cd ~/tigerpaw && ./docker-setup.sh"
    echo ""
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d tigerpaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart tigerpaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f tigerpaw-gateway
}
...
clawdock-workspace() {
  cd ~/.tigerpaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec tigerpaw-gateway \
    bash -c 'echo "alias tigerpaw=\"./tigerclaw.mjs\"" > /tmp/.bashrc_tigerpaw && bash --rcfile /tmp/.bashrc_tigerpaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm tigerpaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build tigerpaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "OPENCLAW_GATEWAY_TOKEN=$token" tigerpaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" tigerpaw-gateway \
    bash -c './tigerclaw.mjs config set gateway.remote.token "$TOKEN" && ./tigerclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec tigerpaw-gateway \
    bash -c "./tigerclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart tigerpaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm tigerpaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec tigerpaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'tigerpaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}ClawDock - Docker Helpers for Tigerpaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (tigerpaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to tigerpaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.tigerpaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'tigerpaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'tigerpaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.tigerpaw.dev${_CLR_RESET}"
  echo ""
}
