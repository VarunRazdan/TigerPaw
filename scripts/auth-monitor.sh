...
#
# Suggested cron: */30 * * * * /home/admin/tigerpaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Tigerpaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., tigerpaw-alerts)
...
STATE_FILE="$HOME/.tigerpaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Tigerpaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use tigerpaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Tigerpaw to $NOTIFY_PHONE..."
            tigerpaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Tigerpaw is down. Run: ssh l36 '~/tigerpaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
