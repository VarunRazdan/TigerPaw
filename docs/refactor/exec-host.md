...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.tigerpaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.tigerpaw/node.json` (node id + pairing token).
...
Path: `~/.tigerpaw/exec-approvals.json`
...
- Unix socket at `~/.tigerpaw/exec-approvals.sock` (0600).
