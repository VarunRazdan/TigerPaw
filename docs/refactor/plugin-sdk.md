...
- Publish as `tigerpaw/plugin-sdk` (or export from core under `tigerpaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `TigerpawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: TigerpawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: TigerpawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: TigerpawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: TigerpawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: TigerpawConfig): string;
  };
};
```
...
- Introduce `tigerpaw/plugin-sdk`.
- Add `api.runtime` to `TigerpawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `tigerpawRuntime: ">=2026.2.0"`).
