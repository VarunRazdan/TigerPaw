---
summary: "VPS hosting hub for Tigerpaw (Oracle/Fly/Hetzner/GCP/exe.dev)"
read_when:
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/tigerpaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- Add service env for stable startup path:
  ...
  - `NODE_COMPILE_CACHE=/var/tmp/tigerpaw-compile-cache`
