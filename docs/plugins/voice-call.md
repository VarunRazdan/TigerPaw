...
read_when:
  - You want to place an outbound voice call from Tigerpaw
...
Voice calls for Tigerpaw via a plugin. Supports outbound notifications and
...
- Use `tigerpaw voicecall ...` or the `voice_call` tool
...
```bash
tigerpaw plugins install ./extensions/voice-call
