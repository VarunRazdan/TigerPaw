...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/tigerpaw`)
...
If the page fails to load, start the Gateway first: `tigerpaw gateway`.
...
you revoke it with `tigerpaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.tigerpaw/tigerpaw.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop tigerpaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Tigerpaw
...
Tigerpaw **blocks** Control UI connections without device identity.
