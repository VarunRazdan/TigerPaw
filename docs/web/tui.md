...
```bash
tigerpaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `tigerpaw tui --deliver`
...
- If your terminal uses a light background and auto-detection is wrong, set `OPENCLAW_THEME=light` before launching `tigerpaw tui`.
...
- Check the Gateway logs: `tigerpaw logs --follow`.
- Confirm the agent can run: `tigerpaw status` and `tigerpaw models status`.
...
- No agents in picker: check `tigerpaw agents list` and your routing config.
