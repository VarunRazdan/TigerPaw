...
The onboarding wizard is the **recommended** way to set up Tigerpaw on macOS,
...
`tigerpaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`tigerpaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.tigerpaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `tigerpaw doctor` first.
</Note>
...
Use `tigerpaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.tigerpaw/workspace-<agentId>`.
...
- CLI command reference: [`tigerpaw onboard`](/cli/onboard)
