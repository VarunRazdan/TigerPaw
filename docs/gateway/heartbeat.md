...
- During heartbeat runs, Tigerpaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `tigerpaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Tigerpaw skips the heartbeat run entirely (no model call).
...
```bash
tigerpaw system event --text "Check for urgent follow-ups" --mode now
```
