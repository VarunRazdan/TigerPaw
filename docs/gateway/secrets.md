...
Tigerpaw supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Tigerpaw runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Tigerpaw validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Tigerpaw QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/tigerpaw"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `tigerpaw.json` refs)
...
When reload-time activation fails after a healthy state, Tigerpaw enters degraded secrets state.
...
- Strict command paths (for example `tigerpaw memory` remote-memory paths and `tigerpaw qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `tigerpaw status`, `tigerpaw status --all`, `tigerpaw channels status`, `tigerpaw channels resolve`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `tigerpaw secrets reload`.
...
```bash
tigerpaw secrets audit --check
tigerpaw secrets configure
tigerpaw secrets audit --check
```
...
- plaintext values at rest (`tigerpaw.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `tigerpaw.json` refs)
...
- lets you select supported secret-bearing fields in `tigerpaw.json` plus `auth-profiles.json` for one agent scope
...
- `tigerpaw secrets configure --providers-only`
- `tigerpaw secrets configure --skip-provider-setup`
- `tigerpaw secrets configure --agent <id>`
...
```bash
tigerpaw secrets apply --from /tmp/tigerpaw-secrets-plan.json
tigerpaw secrets apply --from /tmp/tigerpaw-secrets-plan.json --dry-run
```
...
Tigerpaw intentionally does not write rollback backups containing historical plaintext secret values.
