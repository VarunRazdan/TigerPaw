...
description: "Complete field-by-field reference for ~/.tigerpaw/tigerpaw.json"
summary: "Complete reference for every Tigerpaw config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.tigerpaw/tigerpaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Tigerpaw uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `tigerpaw doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `tigerpaw doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["tigerpaw-dm"] },
      guilds: {
        "123456789012345678": {
...
- Tigerpaw additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `tigerpaw plugins install @tigerpaw/mattermost`.
...
- `commands.callbackUrl` must resolve to the Tigerpaw gateway endpoint and be reachable from the Mattermost server.
...
Tigerpaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `tigerpaw channels add` (or channel onboarding) while still on a single-account top-level channel config, Tigerpaw moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `tigerpaw doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@tigerpaw"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `tigerpaw.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Tigerpaw auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Tigerpaw assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "tigerpaw-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.tigerpaw/sandboxes`
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `tigerpaw.json`.
noVNC observer access uses VNC auth by default and Tigerpaw emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `tigerpaw-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        agentDir: "~/.tigerpaw/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@tigerpaw"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
For `type: "acp"` entries, Tigerpaw resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Tigerpaw starts a fresh thread session instead of inheriting parent transcript history.
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Tigerpaw treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:tigerpaw`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.tigerpaw/attachments/<uuid>/` with a `.manifest.json`.
...
Tigerpaw uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.tigerpaw/agents/<agentId>/agent/models.json`.
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Tigerpaw forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `tigerpaw onboard --auth-choice opencode-zen` or `tigerpaw onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `tigerpaw onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `tigerpaw onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `tigerpaw onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `tigerpaw onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `tigerpaw onboard --auth-choice minimax-api`.
...
- Loaded from `~/.tigerpaw/extensions`, `<workspace>/.tigerpaw/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `tigerpaw plugins update`.
...
Convenience flags: `--dev` (uses `~/.tigerpaw-dev` + port `19001`), `--profile <name>` (uses `~/.tigerpaw-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.tigerpaw/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-tigerpaw-token: <token>`.
...
- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__tigerpaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__tigerpaw__/a2ui/`
...
- Also serves A2UI at `/__tigerpaw__/a2ui/`.
...
- Hostname defaults to `tigerpaw`. Override with `OPENCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.tigerpaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `tigerpaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.tigerpaw/.env` (neither overrides existing vars).
...
- `secrets apply` targets supported `tigerpaw.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.tigerpaw/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/tigerpaw-vault-resolver",
...
- Legacy OAuth imports from `~/.tigerpaw/credentials/oauth.json`.
...
- Default log file: `/tmp/tigerpaw/tigerpaw-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Tigerpaw.`).
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `tigerpaw doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `tigerpaw.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
