...
Tigerpaw supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `tigerpaw gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.tigerpaw/.env` so the daemon can read it:

```bash
cat >> ~/.tigerpaw/.env <<'EOF'
<PROVIDER>_API_KEY=...
EOF
```
...
API keys for daemon use: `tigerpaw onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.tigerpaw/.env`, systemd/launchd).
...
```bash
tigerpaw models auth paste-token --provider anthropic
tigerpaw models auth paste-token --provider openrouter
```
...
- Tigerpaw retries with the next key only for rate-limit errors (for example
...
Run `tigerpaw models status` to confirm which profile is expiring. If the profile
