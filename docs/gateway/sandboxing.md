---
summary: "How Tigerpaw sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
Tigerpaw can run **tools inside Docker containers** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`tigerpaw-sandbox-browser`) instead of the global `bridge` network.
  ...
  - noVNC observer access is password-protected by default; Tigerpaw emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
- `"none"` (default): tools see a sandbox workspace under `~/.tigerpaw/sandboxes`.
- `"ro"`: mounts the agent workspace read-only at `/agent` (disables `write`/`edit`/`apply_patch`).
- `"rw"`: mounts the agent workspace read/write at `/workspace`.

Inbound media is copied into the active sandbox workspace (`media/inbound/*`).
Skills note: the `read` tool is sandbox-rooted. With `workspaceAccess: "none"`,
Tigerpaw mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- Tigerpaw blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
...
Default image: `tigerpaw-sandbox:bookworm-slim`
...
`tigerpaw-sandbox-common:bookworm-slim`.
...
- Use `tigerpaw sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
