...
- `tigerpaw gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `tigerpaw doctor` reports no blocking config/service issues.
- `tigerpaw channels status --probe` shows connected/ready channels.
...
tigerpaw config get agents.defaults.models
```
...
tigerpaw status
tigerpaw channels status --probe
tigerpaw pairing list --channel <channel> [--account <id>]
tigerpaw config get channels
tigerpaw logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `tigerpaw config get gateway.auth.token` then paste into Control UI settings.                          |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `tigerpaw devices list` then `tigerpaw devices approve <requestId>`.                                                                        |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `tigerpaw configure`). If you are running Tigerpaw via Podman using the dedicated `tigerpaw` user, the config lives at `~tigerpaw/.tigerpaw/tigerpaw.json`.
...
```bash
tigerpaw cron status
tigerpaw cron list
tigerpaw cron runs --id <jobId> --limit 20
tigerpaw system heartbeat last
tigerpaw logs --follow
```
...
tigerpaw approvals get --node <idOrNameOrIp>
tigerpaw logs --follow
tigerpaw status
```
...
tigerpaw browser start --browser-profile tigerpaw
...
```bash
tigerpaw gateway status
tigerpaw config get gateway.mode
tigerpaw config get gateway.remote.url
tigerpaw config get gateway.auth.mode
```
...
```bash
tigerpaw config get gateway.bind
tigerpaw config get gateway.auth.token
tigerpaw gateway status
tigerpaw logs --follow
```
