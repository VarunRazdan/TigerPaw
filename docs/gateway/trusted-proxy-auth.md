...
read_when:
  - Running Tigerpaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Tigerpaw
...
- You run Tigerpaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Tigerpaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Tigerpaw extracts the user identity from the configured header
...
- Tigerpaw can stay on loopback HTTP behind the proxy.
...
If Tigerpaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://tigerpaw.example.com
    to: http://tigerpaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
tigerpaw.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy tigerpaw:18789 {
...
location / {
    ...
    proxy_pass http://tigerpaw:18789;
...
`tigerpaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Tigerpaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `tigerpaw security audit` and review findings
