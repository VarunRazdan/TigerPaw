...
read_when:
  ...
  - Wiring external systems into Tigerpaw
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Tigerpaw config directory (typically `~/.tigerpaw/hooks/transforms`).
...
- `tigerpaw webhooks gmail setup` writes `hooks.gmail` config for `tigerpaw webhooks gmail run`.
