...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a Tigerpaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`tigerpaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Tigerpaw gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
tigerpaw acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
tigerpaw acp --url wss://gateway-host:18789 --token-file ~/.tigerpaw/gateway.token

# Attach to an existing session key
tigerpaw acp --session agent:main:main

# Attach by label (must already exist)
tigerpaw acp --session-label "support inbox"

# Reset the session key before the first prompt
tigerpaw acp --session agent:main:main --reset-session
```
...
tigerpaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.tigerpaw/gateway.token

# Override the server command (default: tigerpaw)
tigerpaw acp client --server "node" --server-args tigerpaw.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive a Tigerpaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `tigerpaw acp` over stdio.

Example config (persisted):

```bash
tigerpaw config set gateway.remote.url wss://gateway-host:18789
tigerpaw config set gateway.remote.token <token>
```
...
```bash
tigerpaw acp --session agent:main:main
tigerpaw acp --session agent:design:main
tigerpaw acp --session agent:qa:bug-123
```
...
Tigerpaw bot over ACP, use `acpx` with its built-in `tigerpaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx tigerpaw` at `tigerpaw acp`.
3. Target the Tigerpaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Tigerpaw ACP session
acpx tigerpaw exec "Summarize the active Tigerpaw session state."

# Persistent named session for follow-up turns
acpx tigerpaw sessions ensure --name codex-bridge
acpx tigerpaw -s codex-bridge --cwd /path/to/repo \
  "Ask my Tigerpaw work agent for recent context relevant to this repo."
```

If you want `acpx tigerpaw` to target a specific Gateway and session key every
time, override the `tigerpaw` agent command in `~/.acpx/config.json`:
...
For a repo-local Tigerpaw checkout, use the direct CLI entrypoint instead of the
...
In Zed, open the Agent panel and select “Tigerpaw ACP” to start a thread.
...
- `--server <command>`: ACP server command (default: `tigerpaw`).
