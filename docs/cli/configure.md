---
summary: "CLI reference for `tigerpaw configure` (interactive configuration prompts)"
read_when:
...
Tip: `tigerpaw config` without a subcommand opens the same wizard. Use
`tigerpaw config get|set|unset` for non-interactive edits.
...
```bash
tigerpaw configure
tigerpaw configure --section model --section channels
```
