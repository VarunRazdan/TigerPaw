---
summary: "CLI reference for `tigerpaw onboard` (interactive onboarding wizard)"
read_when:
...
```bash
tigerpaw onboard
tigerpaw onboard --flow quickstart
tigerpaw onboard --flow manual
tigerpaw onboard --mode remote --remote-url wss://gateway-host:18789
```
...
- Fastest first chat: `tigerpaw dashboard` (Control UI, no channel setup).
