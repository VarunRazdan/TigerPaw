...
This plan defines how Tigerpaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- Tigerpaw core owns ACP control plane concerns
...
Tigerpaw should not reimplement ACP transport internals in core.
Tigerpaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in Tigerpaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under Tigerpaw state dir
...
- ACP event stream is normalized to Tigerpaw reply chunks
...
For the first production backend (`extensions/acpx`), Tigerpaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses Tigerpaw `sessionKey`)
...
- Tigerpaw consumes ndjson events from `acpx --format json --json-strict`
