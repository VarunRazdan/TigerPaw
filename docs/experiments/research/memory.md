...
read_when:
  - Designing workspace memory (~/.tigerpaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Tigerpaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.tigerpaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.tigerpaw/workspace` as canonical human-readable memory.
...
```
~/.tigerpaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Tigerpaw**, but keep a separable core library.

### Why integrate into Tigerpaw?

- Tigerpaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `tigerpaw memory recall "…" --k 25 --since 30d`
  - `tigerpaw memory reflect --since 7d`
...
Pragmatic take for `~/.tigerpaw/workspace`:
