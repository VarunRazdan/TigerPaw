---
summary: "Optional Docker-based setup and onboarding for Tigerpaw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Tigerpaw on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full Tigerpaw in Docker)
...
- Need the URL again? Run `docker compose run --rm tigerpaw-cli dashboard --no-open`.
...
```bash
docker compose run -T --rm tigerpaw-cli gateway probe
docker compose run -T --rm tigerpaw-cli devices list --json
```
...
`tigerpaw-cli` uses `network_mode: "service:tigerpaw-gateway"` so CLI commands can
...
network path instead of the bundled `tigerpaw-cli` service.

To reduce impact if the CLI process is compromised, the compose config drops
`NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on `tigerpaw-cli`.
...
- [GitHub Container Registry package](https://github.com/varunrazdan/tigerpaw/pkgs/container/tigerpaw)

Use image name `ghcr.io/varunrazdan/tigerpaw` (not similarly named Docker Hub
images).
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/varunrazdan/tigerpaw/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
```
...
See [`ClawDock` Helper README](https://github.com/varunrazdan/tigerpaw/blob/main/scripts/shell-helpers/README.md) for details.

### Manual flow (compose)

```bash
docker build -t tigerpaw:local -f Dockerfile .
docker compose run --rm tigerpaw-cli onboard
docker compose up -d tigerpaw-gateway
```
...
```bash
docker compose run --rm tigerpaw-cli dashboard --no-open
docker compose run --rm tigerpaw-cli devices list
docker compose run --rm tigerpaw-cli devices approve <requestId>
```
...
`/home/node/.tigerpaw`, make sure your host bind mounts are owned by uid 1000.

Example (Linux host):

```bash
sudo chown -R 1000:1000 /path/to/tigerpaw-config /path/to/tigerpaw-workspace
```
...
```bash
docker compose run --rm tigerpaw-cli channels login
```

Telegram (bot token):

```bash
docker compose run --rm tigerpaw-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):

```bash
docker compose run --rm tigerpaw-cli channels add --channel discord --token "<token>"
```
...
background. In plain terms: Docker keeps checking if Tigerpaw is still
...
```bash
docker compose run --rm tigerpaw-cli config set gateway.mode local
docker compose run --rm tigerpaw-cli config set gateway.bind lan
docker compose run --rm tigerpaw-cli devices list --url ws://127.0.0.1:18789
```
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.tigerpaw/sandboxes`
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  Tigerpaw auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `tigerpaw sandbox recreate ...` command.
...
This builds `tigerpaw-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `tigerpaw-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "tigerpaw-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
This builds `tigerpaw-sandbox-browser:bookworm-slim` using
...
- Browser containers default to a dedicated Docker network (`tigerpaw-sandbox-browser`) instead of global `bridge`.
...
- noVNC observer access is password-protected by default; Tigerpaw provides a short-lived observer token URL that serves a local bootstrap page and keeps the password in URL fragment (instead of URL query).
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-tigerpaw-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-tigerpaw-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-tigerpaw-sbx" } },
    },
  },
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/varunrazdan/tigerpaw/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: Tigerpaw runs commands with `sh -lc` (login shell), which
