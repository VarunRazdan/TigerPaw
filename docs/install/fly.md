---
title: Fly.io
description: Deploy Tigerpaw on Fly.io
summary: "Step-by-step Fly.io deployment for Tigerpaw with persistent storage and HTTPS"
read_when:
  - Deploying Tigerpaw on Fly.io
  - Setting up Fly volumes, secrets, and first-run config
---

# Fly.io Deployment

**Goal:** Tigerpaw Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
fly apps create my-tigerpaw

# Create a persistent volume (1GB is usually enough)
fly volumes create tigerpaw_data --size 1 --region iad
```
...
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `tigerpaw.json` where they could be accidentally exposed or logged.
...
**Note:** With `OPENCLAW_STATE_DIR=/data`, the config path is `/data/tigerpaw.json`.
...
Or visit `https://my-tigerpaw.fly.dev/`
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/tigerpaw.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/tigerpaw.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/tigerpaw.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/tigerpaw.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/tigerpaw.json"
```
...
```bash
# List current IPs
fly ips list -a my-tigerpaw

# Release public IPs
fly ips release <public-ipv4> -a my-tigerpaw
fly ips release <public-ipv6> -a my-tigerpaw
...
fly ips allocate-v6 --private -a my-tigerpaw
```
...
fly proxy 3000:3000 -a my-tigerpaw
