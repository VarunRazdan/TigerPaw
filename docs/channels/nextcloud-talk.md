...
```bash
tigerpaw plugins install @tigerpaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
tigerpaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Tigerpaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `tigerpaw pairing list nextcloud-talk`
  - `tigerpaw pairing approve nextcloud-talk <CODE>`
