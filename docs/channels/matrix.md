...
Matrix is an open, decentralized messaging protocol. Tigerpaw connects as a Matrix **user**
...
```bash
tigerpaw plugins install @tigerpaw/matrix
```

Local checkout (when running from a git repo):

```bash
tigerpaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `tigerpaw plugins install @tigerpaw/matrix`
   - From a local checkout: `tigerpaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Tigerpaw calls the same
     login endpoint, stores the access token in `~/.tigerpaw/credentials/matrix/credentials.json`,
...
- On first connection, Tigerpaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Tigerpaw logs a warning.
...
`~/.tigerpaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `tigerpaw pairing list matrix`
  - `tigerpaw pairing approve matrix <CODE>`
...
- On startup, Tigerpaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
