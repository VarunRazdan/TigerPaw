...
Tlon is a decentralized messenger built on Urbit. Tigerpaw connects to your Urbit ship and can
...
```bash
tigerpaw plugins install @tigerpaw/tlon
```

Local checkout (when running from a git repo):

```bash
tigerpaw plugins install ./extensions/tlon
```
...
By default, Tigerpaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `tigerpaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Tigerpaw replies in-thread.
