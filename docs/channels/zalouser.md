...
read_when:
  - Setting up Zalo Personal for Tigerpaw
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Tigerpaw.
...
- Install via CLI: `tigerpaw plugins install @tigerpaw/zalouser`
- Or from a source checkout: `tigerpaw plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `tigerpaw channels login --channel zalouser`
...
```bash
tigerpaw directory self --channel zalouser
tigerpaw directory peers list --channel zalouser --query "name"
tigerpaw directory groups list --channel zalouser --query "work"
```
...
- `tigerpaw pairing list zalouser`
- `tigerpaw pairing approve zalouser <code>`
...
- On startup, Tigerpaw resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- When a group message is skipped because mention is required, Tigerpaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Tigerpaw state. Example:
...
- Tigerpaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Tigerpaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `tigerpaw channels logout --channel zalouser && tigerpaw channels login --channel zalouser`
...
- The channel now runs fully in Tigerpaw without external CLI binaries.
