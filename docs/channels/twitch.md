...
read_when:
  - Setting up Twitch chat integration for Tigerpaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Tigerpaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
tigerpaw plugins install @tigerpaw/twitch
```

Local checkout (when running from a git repo):

```bash
tigerpaw plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "tigerpaw", // Bot's Twitch account
