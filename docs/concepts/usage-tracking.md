...
- `/usage cost` in chats: local cost summary aggregated from Tigerpaw session logs.
- CLI: `tigerpaw status --usage` prints a full per-provider breakdown.
- CLI: `tigerpaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
