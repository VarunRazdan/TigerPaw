---
summary: "Use MiniMax M2.5 in Tigerpaw"
read_when:
  - You want MiniMax models in Tigerpaw
...
```bash
tigerpaw plugins enable minimax-portal-auth  # skip if already loaded.
tigerpaw gateway restart  # restart if gateway is already running
tigerpaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/varunrazdan/tigerpaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `tigerpaw.json`:
...
- Use `tigerpaw models list` and `tigerpaw models set minimax/MiniMax-M2.5` to switch.
...
- Running `tigerpaw configure` and selecting **MiniMax M2.5**, or
