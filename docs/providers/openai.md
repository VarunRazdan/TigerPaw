---
summary: "Use OpenAI via API keys or Codex subscription in Tigerpaw"
read_when:
  - You want to use OpenAI models in Tigerpaw
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Tigerpaw.
...
```bash
tigerpaw onboard --auth-choice openai-api-key
# or non-interactive
tigerpaw onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. Tigerpaw forwards both through the `openai/*` Responses path.
...
tigerpaw onboard --auth-choice openai-codex

# Or run OAuth directly
tigerpaw models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. Tigerpaw
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

### Transport default

Tigerpaw uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Tigerpaw also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Tigerpaw enables it by default for
...
Tigerpaw, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
`baseUrl` on `api.openai.com`), Tigerpaw now auto-enables OpenAI server-side
