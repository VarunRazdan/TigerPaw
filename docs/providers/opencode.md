---
summary: "Use OpenCode Zen and Go catalogs with Tigerpaw"
read_when:
...
OpenCode exposes two hosted catalogs in Tigerpaw:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Tigerpaw keeps the runtime provider ids
...
tigerpaw onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
tigerpaw onboard --auth-choice opencode-go
tigerpaw onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
