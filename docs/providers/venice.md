---
summary: "Use Venice AI privacy-focused models in Tigerpaw"
read_when:
  - You want privacy-focused inference in Tigerpaw
...
```bash
tigerpaw onboard --auth-choice venice-api-key
```
...
```bash
tigerpaw agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Tigerpaw shows all available Venice models. Pick based on your needs:
...
```bash
tigerpaw models set venice/kimi-k2-5
tigerpaw models set venice/claude-opus-4-6
```

List all available models:

```bash
tigerpaw models list | grep venice
```

## Configure via `tigerpaw configure`

1. Run `tigerpaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Tigerpaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
tigerpaw agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
tigerpaw agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
tigerpaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
tigerpaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
tigerpaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `tigerpaw models list` to see currently available models. Some models may be temporarily offline.
