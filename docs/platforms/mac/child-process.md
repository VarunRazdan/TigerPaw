...
service via the external `tigerpaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.tigerpaw.gateway`
  (or `ai.tigerpaw.<profile>` when using `--profile`/`OPENCLAW_PROFILE`; legacy `com.tigerpaw.*` is supported).
