...
- Same path: run the app from a fixed location (for Tigerpaw, `dist/Tigerpaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.tigerpaw.mac
sudo tccutil reset ScreenCapture ai.tigerpaw.mac
...
Workaround: move files into the Tigerpaw workspace (`~/.tigerpaw/workspace`) if you want to avoid per-folder grants.
