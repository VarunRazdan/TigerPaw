---
summary: "Tigerpaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Tigerpaw on a Raspberry Pi
  - Running Tigerpaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Tigerpaw on Raspberry Pi

## Goal

Run a persistent, always-on Tigerpaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u tigerpaw -f
```

## 9) Access the Tigerpaw Dashboard
...
```bash
ssh user@gateway-host 'tigerpaw dashboard --no-open'
```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/tigerpaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/tigerpaw-compile-cache
mkdir -p /var/tmp/tigerpaw-compile-cache
...
If this Pi is mostly running Tigerpaw, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit tigerpaw
```

```ini
[Service]
Environment=OPENCLAW_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/tigerpaw-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Tigerpaw state/cache on SSD-backed storage to avoid SD-card
...
Most Tigerpaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled tigerpaw

# Enable if not
sudo systemctl enable tigerpaw

# Start on boot
sudo systemctl start tigerpaw
```
...
```bash
# Check logs
journalctl -u tigerpaw --no-pager -n 100

# Common fix: rebuild
cd ~/tigerpaw  # if using hackable install
npm run build
sudo systemctl restart tigerpaw
```
