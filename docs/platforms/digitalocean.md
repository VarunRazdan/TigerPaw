---
summary: "Tigerpaw on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Tigerpaw on DigitalOcean
  - Looking for cheap VPS hosting for Tigerpaw
title: "DigitalOcean"
---

# Tigerpaw on DigitalOcean

## Goal

Run a persistent Tigerpaw Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status tigerpaw-gateway.service

# View logs
journalctl --user -u tigerpaw-gateway.service -f
```
...
```bash
tigerpaw channels login whatsapp
# Scan QR code
```
...
- `~/.tigerpaw/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u tigerpaw --no-pager -n 50
```
