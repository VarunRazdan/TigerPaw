...
This is the full reference for the `tigerpaw onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.tigerpaw/credentials/oauth.json`; auth profiles live in `~/.tigerpaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.tigerpaw/credentials/oauth.json` (or `$OPENCLAW_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.tigerpaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `tigerpaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `tigerpaw configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `tigerpaw health`.
    - Tip: `tigerpaw status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
- Stores it under `~/.tigerpaw/tools/signal-cli/<version>/`.
...
Typical fields in `~/.tigerpaw/tigerpaw.json`:
...
`tigerpaw agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.tigerpaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.tigerpaw/agents/<agentId>/sessions/`.
