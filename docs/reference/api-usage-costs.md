...
Tigerpaw features that can generate provider usage or paid API calls.
...
- `tigerpaw status --usage` and `tigerpaw channels list` show provider **usage windows**
...
Tigerpaw can pick up credentials from:
...
- `tigerpaw status --usage`
- `tigerpaw models status --json`
...
`tigerpaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
