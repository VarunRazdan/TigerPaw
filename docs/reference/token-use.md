---
summary: "How Tigerpaw builds prompt context and reports token usage + costs"
read_when:
...
Tigerpaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Tigerpaw assembles its own system prompt on every run. It includes:
...
For images, Tigerpaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Tigerpaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `tigerpaw status --usage` and `tigerpaw channels list` show
...
`cacheWrite`. If pricing is missing, Tigerpaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Tigerpaw can
...
Anthropic's 1M context window is currently beta-gated. Tigerpaw can inject the
...
Tigerpaw skips the `context-1m-*` beta header because Anthropic currently
