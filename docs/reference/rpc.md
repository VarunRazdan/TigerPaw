...
Tigerpaw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Tigerpaw owns lifecycle when `channels.signal.autoStart=true`.
...
- Tigerpaw spawns `imsg rpc` as a child process (legacy iMessage integration).
