...
- With Anthropic API-key auth profiles, Tigerpaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Tigerpaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Tigerpaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.tigerpaw/logs/cache-trace.jsonl" # optional
