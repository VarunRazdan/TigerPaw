...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Tigerpaw
...
Running Tigerpaw via Node with `tsx` fails at startup with:

```
[tigerpaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `tigerpaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch tigerpaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node tigerpaw.mjs status` works on Node 25.
