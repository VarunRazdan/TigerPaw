...
- `commands.config` (default `false`) enables `/config` (reads/writes `tigerpaw.json`).
...
- For full provider usage breakdown, use `tigerpaw status --usage`.
- `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
- In multi-account channels, config-targeted `/allowlist --account <id>` and `/config set channels.<provider>.accounts.<id>...` also honor the target account's `configWrites`.
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from Tigerpaw session logs.
...
```
/debug show
/debug set messages.responsePrefix="[tigerpaw]"
...
- Overrides apply immediately to new config reads, but do **not** write to `tigerpaw.json`.
- Use `/debug reset` to clear all overrides and return to the on-disk config.

## Config updates

`/config` writes to your on-disk config (`tigerpaw.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[tigerpaw]"
...
- **Slack:** `channels.slack.slashCommand` is still supported for a single `/tigerpaw`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
