---
summary: "Chrome extension: let Tigerpaw drive your existing Chrome tab"
read_when:
...
The Tigerpaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate tigerpaw-managed Chrome profile.
...
Tigerpaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Tigerpaw release (npm package) as static files. There is no separate “build” step.

After upgrading Tigerpaw:

- Re-run `tigerpaw browser extension install` to refresh the installed files under your Tigerpaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Tigerpaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Tigerpaw to control.
...
- `ON`: attached; Tigerpaw can drive that tab.
...
Debugging: `tigerpaw sandbox explain`
...
`tigerpaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `tigerpaw browser extension install` first to copy the extension to a stable location under your Tigerpaw state directory.
...
- **This is not isolated** like the dedicated tigerpaw-managed profile.
