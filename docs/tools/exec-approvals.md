...
- **gateway host** → `tigerpaw` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Tigerpaw also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.tigerpaw/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.tigerpaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `tigerpaw approvals allowlist ...`).
- `tigerpaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `tigerpaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.tigerpaw/exec-approvals.json` directly.

CLI: `tigerpaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, Tigerpaw does not treat
...
topics, Tigerpaw preserves the topic for the approval prompt and the post-approval follow-up.
