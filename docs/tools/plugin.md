---
summary: "Tigerpaw plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends Tigerpaw with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core Tigerpaw yet (or you want to keep optional features out of your main
install).
...
```bash
tigerpaw plugins install @tigerpaw/voice-call
```
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@tigerpaw/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@tigerpaw/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@tigerpaw/zalouser`
- [Matrix](/channels/matrix) — `@tigerpaw/matrix`
- [Nostr](/channels/nostr) — `@tigerpaw/nostr`
- [Zalo](/channels/zalo) — `@tigerpaw/zalo`
- [Microsoft Teams](/channels/msteams) — `@tigerpaw/msteams`
- Google Antigravity OAuth (provider auth) — bundled as `google-antigravity-auth` (disabled by default)
- Gemini CLI OAuth (provider auth) — bundled as `google-gemini-cli-auth` (disabled by default)
- Qwen OAuth (provider auth) — bundled as `qwen-portal-auth` (disabled by default)
- Copilot Proxy (provider auth) — local VS Code Copilot Proxy bridge; distinct from built-in `github-copilot` device login (bundled, disabled by default)

Tigerpaw plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
Use SDK subpaths instead of the monolithic `tigerpaw/plugin-sdk` import when
authoring plugins:

- `tigerpaw/plugin-sdk/core` for generic plugin APIs, provider auth types, and shared helpers.
- `tigerpaw/plugin-sdk/compat` for bundled/internal plugin code that needs broader shared runtime helpers than `core`.
- `tigerpaw/plugin-sdk/telegram` for Telegram channel plugins.
- `tigerpaw/plugin-sdk/discord` for Discord channel plugins.
- `tigerpaw/plugin-sdk/slack` for Slack channel plugins.
- `tigerpaw/plugin-sdk/signal` for Signal channel plugins.
- `tigerpaw/plugin-sdk/imessage` for iMessage channel plugins.
- `tigerpaw/plugin-sdk/whatsapp` for WhatsApp channel plugins.
- `tigerpaw/plugin-sdk/line` for LINE channel plugins.
- `tigerpaw/plugin-sdk/msteams` for the bundled Microsoft Teams plugin surface.
- Bundled extension-specific subpaths are also available:
  `tigerpaw/plugin-sdk/acpx`, `tigerpaw/plugin-sdk/bluebubbles`,
  `tigerpaw/plugin-sdk/copilot-proxy`, `tigerpaw/plugin-sdk/device-pair`,
  `tigerpaw/plugin-sdk/diagnostics-otel`, `tigerpaw/plugin-sdk/diffs`,
  `tigerpaw/plugin-sdk/feishu`,
  `tigerpaw/plugin-sdk/google-gemini-cli-auth`, `tigerpaw/plugin-sdk/googlechat`,
  `tigerpaw/plugin-sdk/irc`, `tigerpaw/plugin-sdk/llm-task`,
  `tigerpaw/plugin-sdk/lobster`, `tigerpaw/plugin-sdk/matrix`,
  `tigerpaw/plugin-sdk/mattermost`, `tigerpaw/plugin-sdk/memory-core`,
  `tigerpaw/plugin-sdk/memory-lancedb`,
  `tigerpaw/plugin-sdk/minimax-portal-auth`,
  `tigerpaw/plugin-sdk/nextcloud-talk`, `tigerpaw/plugin-sdk/nostr`,
  `tigerpaw/plugin-sdk/open-prose`, `tigerpaw/plugin-sdk/phone-control`,
  `tigerpaw/plugin-sdk/qwen-portal-auth`, `tigerpaw/plugin-sdk/synology-chat`,
  `tigerpaw/plugin-sdk/talk-voice`, `tigerpaw/plugin-sdk/test-utils`,
  `tigerpaw/plugin-sdk/thread-ownership`, `tigerpaw/plugin-sdk/tlon`,
  `tigerpaw/plugin-sdk/twitch`, `tigerpaw/plugin-sdk/voice-call`,
  `tigerpaw/plugin-sdk/zalo`, and `tigerpaw/plugin-sdk/zalouser`.

Compatibility note:

- `tigerpaw/plugin-sdk` remains supported for existing external plugins.
...
- Read-only command paths such as `tigerpaw status`, `tigerpaw status --all`,
  `tigerpaw channels status`, `tigerpaw channels resolve`, and doctor/config
...
- `<workspace>/.tigerpaw/extensions/*.ts`
- `<workspace>/.tigerpaw/extensions/*/index.ts`

3. Global extensions

- `~/.tigerpaw/extensions/*.ts`
- `~/.tigerpaw/extensions/*/index.ts`

4. Bundled extensions (shipped with Tigerpaw, mostly disabled by default)

- `<tigerpaw>/extensions/*`

Most bundled plugins must be enabled explicitly via
`plugins.entries.<id>.enabled` or `tigerpaw plugins enable <id>`.
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, Tigerpaw logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. Tigerpaw blocks candidates when:
...
Each plugin must include a `tigerpaw.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `tigerpaw.extensions`:
...
Security guardrail: every `tigerpaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `tigerpaw plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `tigerpaw.channel` and
install hints via `tigerpaw.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@tigerpaw/nextcloud-talk",
  "tigerpaw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@tigerpaw/nextcloud-talk",
...
Tigerpaw can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.tigerpaw/mpm/plugins.json`
- `~/.tigerpaw/mpm/catalog.json`
- `~/.tigerpaw/plugins/catalog.json`

Or point `OPENCLAW_PLUGIN_CATALOG_PATHS` (or `OPENCLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "tigerpaw": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, Tigerpaw uses it but warns when it doesn’t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  `tigerpaw.plugin.json` (`configSchema`).
...
Tigerpaw augments `uiHints` at runtime based on discovered plugins:
...
tigerpaw plugins install <path>                 # copy a local file/dir into ~/.tigerpaw/extensions/<id>
tigerpaw plugins install ./extensions/voice-call # relative path ok
tigerpaw plugins install ./plugin.tgz           # install from a local tarball
tigerpaw plugins install ./plugin.zip           # install from a local zip
tigerpaw plugins install -l ./extensions/voice-call # link (no copy) for dev
tigerpaw plugins install @tigerpaw/voice-call # install from npm
tigerpaw plugins install @tigerpaw/voice-call --pin # store exact resolved name@version
...
If stored integrity metadata changes between updates, Tigerpaw warns and asks for confirmation (use global `--yes` to bypass prompts).

Plugins may also register their own top‑level commands (example: `tigerpaw voicecall`).
...
- Plugin-managed hooks show up in `tigerpaw hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `tigerpaw hooks`; enable/disable the plugin instead.
...
- When disabled, Tigerpaw blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
API-key setup inside Tigerpaw (no external scripts needed).

Register a provider via `api.registerProvider(...)`. Each provider exposes one
or more auth methods (OAuth, API key, device code, etc.). These methods power:

- `tigerpaw models auth login --provider <id> [--method <id>]`
...
- `config`: The current Tigerpaw config
...
- Main package: `tigerpaw` (this repo)
- Plugins: separate npm packages under `@tigerpaw/*` (example: `@tigerpaw/voice-call`)

Publishing contract:

- Plugin `package.json` must include `tigerpaw.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `tigerpaw plugins install <npm-spec>` uses `npm pack`, extracts into `~/.tigerpaw/extensions/<id>/`, and enables it in config.
...
- CLI: `tigerpaw voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `tigerpaw.extensions` points at the built entrypoint (`dist/index.js`).
