---
read_when:
  - 你正在将 Tigerpaw 迁移到新的笔记本电脑/服务器
  - 你想保留会话、认证和渠道登录（WhatsApp 等）
summary: 将 Tigerpaw 安装从一台机器迁移到另一台
title: 迁移指南
x-i18n:
...
本指南将 Tigerpaw Gateway 网关从一台机器迁移到另一台，**无需重新进行新手引导**。

迁移在概念上很简单：

- 复制**状态目录**（`$OPENCLAW_STATE_DIR`，默认：`~/.tigerpaw/`）— 这包括配置、认证、会话和渠道状态。
- 复制你的**工作区**（默认 `~/.tigerpaw/workspace/`）— 这包括你的智能体文件（记忆、提示等）。
...
- `--profile <name>`（通常变成 `~/.tigerpaw-<profile>/`）
...
如果你有多个配置文件/状态目录（例如 `~/.tigerpaw-main`、`~/.tigerpaw-work`），分别归档每个。

### 步骤 1 — 在新机器上安装 Tigerpaw

在**新**机器上，安装 CLI（如果需要还有 Node）：

- 参见：[安装](/install)

在这个阶段，如果新手引导创建了一个新的 `~/.tigerpaw/` 也没关系 — 你将在下一步覆盖它。
...
### 陷阱：只复制 `tigerpaw.json`

`tigerpaw.json` 是不够的。许多提供商在以下位置存储状态：
...
- `tigerpaw status` 显示 Gateway 网关正在运行
...
- [Tigerpaw 在哪里存储数据？](/help/faq#where-does-tigerpaw-store-its-data)
