...
- `~/Library/Application Support/Tigerpaw/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `tigerpaw-canvas://<session>/<path>`

示例：

- `tigerpaw-canvas://main/` → `<canvasRoot>/main/index.html`
- `tigerpaw-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `tigerpaw-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
tigerpaw nodes canvas present --node <id>
tigerpaw nodes canvas navigate --node <id> --url "/"
tigerpaw nodes canvas eval --node <id> --js "document.title"
tigerpaw nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__tigerpaw__/a2ui/
```
...
tigerpaw nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
tigerpaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "tigerpaw://agent?message=Review%20this%20design";
```
