...
你可以在 `~/.tigerpaw/tigerpaw.json` 中覆盖此设置：
...
所有日志配置都在 `~/.tigerpaw/tigerpaw.json` 的 `logging` 下。
...
- Tigerpaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `tigerpaw plugins enable diagnostics-otel` 启用插件。
...
- `tigerpaw.tokens`（计数器，属性：`tigerpaw.token`、`tigerpaw.channel`、`tigerpaw.provider`、`tigerpaw.model`）
- `tigerpaw.cost.usd`（计数器，属性：`tigerpaw.channel`、`tigerpaw.provider`、`tigerpaw.model`）
- `tigerpaw.run.duration_ms`（直方图，属性：`tigerpaw.channel`、`tigerpaw.provider`、`tigerpaw.model`）
- `tigerpaw.context.tokens`（直方图，属性：`tigerpaw.context`、`tigerpaw.channel`、`tigerpaw.provider`、`tigerpaw.model`）

消息流：

- `tigerpaw.webhook.received`（计数器，属性：`tigerpaw.channel`、`tigerpaw.webhook`）
- `tigerpaw.webhook.error`（计数器，属性：`tigerpaw.channel`、`tigerpaw.webhook`）
- `tigerpaw.webhook.duration_ms`（直方图，属性：`tigerpaw.channel`、`tigerpaw.webhook`）
- `tigerpaw.message.queued`（计数器，属性：`tigerpaw.channel`、`tigerpaw.source`）
- `tigerpaw.message.processed`（计数器，属性：`tigerpaw.channel`、`tigerpaw.outcome`）
- `tigerpaw.message.duration_ms`（直方图，属性：`tigerpaw.channel`、`tigerpaw.outcome`）

队列 + 会话：

- `tigerpaw.queue.lane.enqueue`（计数器，属性：`tigerpaw.lane`）
- `tigerpaw.queue.lane.dequeue`（计数器，属性：`tigerpaw.lane`）
- `tigerpaw.queue.depth`（直方图，属性：`tigerpaw.lane` 或 `tigerpaw.channel=heartbeat`）
- `tigerpaw.queue.wait_ms`（直方图，属性：`tigerpaw.lane`）
- `tigerpaw.session.state`（计数器，属性：`tigerpaw.state`、`tigerpaw.reason`）
- `tigerpaw.session.stuck`（计数器，属性：`tigerpaw.state`）
- `tigerpaw.session.stuck_age_ms`（直方图，属性：`tigerpaw.state`）
- `tigerpaw.run.attempt`（计数器，属性：`tigerpaw.attempt`）
...
- `tigerpaw.webhook.error`
  - `tigerpaw.channel`、`tigerpaw.webhook`、`tigerpaw.chatId`、`tigerpaw.error`
- `tigerpaw.message.processed`
  - `tigerpaw.channel`、`tigerpaw.outcome`、`tigerpaw.chatId`、`tigerpaw.messageId`、`tigerpaw.sessionKey`、`tigerpaw.sessionId`、`tigerpaw.reason`
- `tigerpaw.session.stuck`
  - `tigerpaw.state`、`tigerpaw.ageMs`、`tigerpaw.queueDepth`、`tigerpaw.sessionKey`、`tigerpaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `tigerpaw doctor`。
