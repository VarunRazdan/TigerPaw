...
`tigerpaw.invoke --each`) without adding new Tigerpaw code per workflow.
...
This extension depends on Tigerpaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Tigerpaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.tigerpaw/extensions` as a standalone plugin directory.
