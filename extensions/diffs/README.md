...
Read-only diff viewer plugin for **Tigerpaw** agents.
...
Set plugin-wide defaults in `~/.tigerpaw/tigerpaw.json`:
...
Before:
Tigerpaw supports plugins.

After:
Tigerpaw supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/tigerpaw-diffs`).
