# Google Gemini CLI Auth (Tigerpaw plugin)
...
```bash
tigerpaw plugins enable google-gemini-cli-auth
```
...
```bash
tigerpaw models auth login --provider google-gemini-cli --set-default
```
...
- `TIGERPAW_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `TIGERPAW_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
