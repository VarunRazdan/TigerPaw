...
import { resolveOpenClawPackageRoot } from "../infra/tigerpaw-root.js";
...
async function generateCompletionCache(): Promise<boolean> {
  ...
  const binPath = path.join(root, "tigerpaw.mjs");
...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(tigerpaw completion ...)` */
 ...
 */
export async function ensureCompletionCacheExists(binName = "tigerpaw"): Promise<boolean> {
  const shell = resolveShellFromEnv() as CompletionShell;
  const cacheExists = await completionCacheExists(shell, binName);
