...
vi.mock("../tools.js", () => ({
  MCP_TOOLS: [
    ...
    },
    {
      ...
      description: "Toggle kill switch",
      inputSchema: { type: "object", properties: {} },
    },
  ],
...
describe("MCP server auth flow", () => {
  describe("when token is configured", () => {
    ...
    it("rejects tools/call before initialize", async () => {
      const session = createMcpSession(authConfig);
      const res = await session.handleRequest(req("tools/call", { name: "get_trading_state" }));
      expect(res.error?.code).toBe(-32600);
    });
    ...
    it("rejects all requests after failed initialize", async () => {
      const session = createMcpSession(authConfig);
      await session.handleRequest(req("initialize", { _meta: { auth: { token: "wrong" } } }));
    ...
    it("rejects initialize without token when token is required", async () => {
      const session = createMcpSession(authConfig);
      const res = await session.handleRequest(req("initialize", { protocolVersion: "2024-11-05" }));
      expect(res.error?.code).toBe(-32600);
    });
  });
