...
describe("MCP Tools", () => {
  ...
  describe("toggle_kill_switch", () => {
    it("activates kill switch with correct args", async () => {
      ...
      expect(mockActivateKillSwitch).toHaveBeenCalledWith("Risk limit breached", "operator", "soft");
  ...
  describe("get_trade_history", () => {
    it("filters and returns trade-related audit entries", async () => {
      ...
      const parsed = parseResult(result) as { trades: Array<Record<string, unknown>>; total: number };
