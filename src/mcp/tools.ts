...
export const MCP_TOOLS: McpToolDef[] = [
  {
    ...
    description: "Get the current trading state including P&L, positions, kill switch status, and risk metrics.",
    inputSchema: {
      type: "object",
      properties: {},
    },
  ...
  {
    name: "place_order",
    description: "Submit a trading order. Basic risk checks (kill switch, daily limits) are enforced. Note: for full policy engine evaluation, submit orders through the gateway.",
    inputSchema: {
      type: "object",
      properties: {
        extensionId: { type: "string", description: "Trading platform (e.g., alpaca, coinbase, binance)" },
  ...
  },
  {
    ...
    description: "Activate or deactivate the global trading kill switch. When active, all new trades are blocked.",
    inputSchema: {
      type: "object",
      properties: {
        active: { type: "string", description: "Set to 'true' to activate, 'false' to deactivate", enum: ["true", "false"] },
  ...
  },
  {
    ...
    description: "Get recent trade history from the audit log, including order actions, symbols, and sides.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Get current risk state including drawdown, P&L, consecutive losses, and portfolio value. For full computed metrics (Sharpe, Sortino), use run_backtest.",
    inputSchema: {
      type: "object",
      properties: {},
    },
...
export async function executeTool(
  ...
  switch (name) {
    ...
    case "place_order": {
      const extensionId = String(args.extensionId ?? "");
      ...
      if (args.limitPrice !== undefined) {
        validateTradingNumeric("limitPrice", Number(args.limitPrice), 0, MAX_PRICE_USD);
      }
    ...
    case "toggle_kill_switch": {
      ...
      const reason = String(args.reason ?? "Toggled via MCP");
      const mode = (String(args.mode ?? "hard")) as "hard" | "soft";
    ...
    case "get_trade_history": {
      ...
      validateTradingNumeric("limit", limit, 1, MAX_TRADE_HISTORY_LIMIT);
      const entries = await readAuditEntries();
      // Filter to trade-related actions and take the most recent N
      const tradeActions = ["auto_approved", "manually_approved", "submitted", "filled", "denied", "rejected"];
      const trades = entries
        .filter((e) => tradeActions.includes(e.action))
        .slice(-limit)
        .map((e) => ({
    ...
    case "get_risk_metrics": {
      ...
      const drawdownPercent = state.highWaterMarkUsd > 0
        ? (drawdownUsd / state.highWaterMarkUsd) * 100
        : 0;
    ...
    case "list_strategies": {
      ...
      return textResult(JSON.stringify(strategies.map((s) => ({
        id: s.id,
        name: s.name,
        enabled: s.enabled,
        symbols: s.symbols,
    ...
    case "run_backtest": {
      ...
      if (!strategyId) return textResult("Error: strategyId is required.");

      const days = Number(args.days ?? 365);
      const initialCapital = Number(args.initialCapital ?? 10000);
      validateTradingNumeric("days", days, 1, MAX_BACKTEST_DAYS);
      validateTradingNumeric("initialCapital", initialCapital, 1, MAX_INITIAL_CAPITAL_USD);

      const { getStrategy } = await import("../trading/strategies/registry.js");
      const strategy = await getStrategy(strategyId);
      if (!strategy) return textResult(`Error: Strategy not found: ${strategyId}`);
      ...
      const symbol = String(args.symbol ?? strategy.symbols[0] ?? "DEMO");
