...
import { validateTokenStrength } from "../gateway/auth.js";
 ...
 * Validates token strength when a token is present (unless TIGERPAW_ALLOW_WEAK_AUTH=1).
 */
export function resolveMcpAuthConfig(env: Record<string, string | undefined>): McpAuthConfig {
  ...
  if (token && env.TIGERPAW_ALLOW_WEAK_AUTH !== "1") {
    validateTokenStrength(token, "MCP auth token");
  }
