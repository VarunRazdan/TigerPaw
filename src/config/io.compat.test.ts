...
describe("config io paths", () => {
  ...
  it("defaults to ~/.tigerpaw/tigerpaw.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".tigerpaw", "tigerpaw.json"));
    });
  });
  ...
  it("uses OPENCLAW_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      ...
      expect(io.configPath).toBe(path.join(home, "svc-home", ".tigerpaw", "tigerpaw.json"));
    });
  });
