...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["tigerpaw security audit", "Run a local security audit."],
          ["tigerpaw security audit --deep", "Include best-effort live Gateway probe checks."],
          ["tigerpaw security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["tigerpaw security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.tigerpaw.dev/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Tigerpaw security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("tigerpaw security audit --deep")}`));

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("tigerpaw security audit --fix")}`));
        if (!fixResult) {
