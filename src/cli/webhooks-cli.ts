...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.tigerpaw.dev/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Tigerpaw hooks")
    ...
    .option("--hook-url <url>", "Tigerpaw hook URL")
    .option("--hook-token <token>", "Tigerpaw hook token")
