    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "tigerpaw node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["tigerpaw node status", "Check node host service status."],
          ["tigerpaw node install", "Install the node host service."],
          ["tigerpaw node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.tigerpaw.dev/cli/node")}\n`,
    );
