    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["tigerpaw nodes status", "List known nodes with live status."],
          ["tigerpaw nodes pairing pending", "Show pending node pairing requests."],
          ['tigerpaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["tigerpaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.tigerpaw.dev/cli/nodes")}\n`,
    );
