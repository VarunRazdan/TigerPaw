...
${formatHelpExamples([
  ['tigerpaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['tigerpaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'tigerpaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'tigerpaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['tigerpaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'tigerpaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.tigerpaw.dev/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.tigerpaw.dev/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['tigerpaw agents set-identity --agent main --name "Tigerpaw" --emoji "🐅"', "Set name + emoji."],
  ["tigerpaw agents set-identity --agent main --avatar avatars/openclaw.png", "Set avatar path."],
  [
    "tigerpaw agents set-identity --workspace ~/.openclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "tigerpaw agents set-identity --identity-file ~/.openclaw/workspace/IDENTITY.md --agent main",
