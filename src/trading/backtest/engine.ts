...
import type { StrategyDefinition, SignalConfig } from "../strategies/types.js";
...
function computeMetrics(
  ...
  const totalReturn = config.initialCapitalUsd > 0
  ...
  if (n >= 2) {
    ...
    const downsideVar = dailyPnl.reduce((s, v) => s + Math.min(0, v - RISK_FREE_DAILY) ** 2, 0) / (n - 1);
...
export async function runBacktest(
  ...
  log.info(`backtest start: strategy="${strategy.name}" symbol=${config.symbol} bars=${bars.length}`);
  ...
  for (let i = warmupBars; i < bars.length; i++) {
    ...
    const prevEquity = i > warmupBars ? equityCurve[equityCurve.length - 2]?.equity ?? config.initialCapitalUsd : config.initialCapitalUsd;
      ...
      if (aggregateStrength >= strategy.entryRule.minSignalStrength) {
        ...
        switch (strategy.positionSizing.method) {
          ...
          case "percent_portfolio":
            ...
            break;
          case "kelly":
            sizeUsd = portfolioValue * (aggregateStrength * (strategy.positionSizing.percentPortfolio ?? 10) / 100);
            break;
          case "risk_parity":
          default:
            sizeUsd = portfolioValue * ((strategy.positionSizing.percentPortfolio ?? 5) / 100);
  ...
  log.info(`backtest done: ${trades.length} trades, ${metrics.totalReturn.toFixed(1)}% return, ${durationMs}ms`);
