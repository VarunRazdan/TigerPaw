...
import { getStrategy, recordExecution, updateStrategyPerformance } from "./registry.js";
import { evaluateSignals, type MarketSnapshot } from "./signals.js";
import type { StrategyExecution, PositionSizing } from "./types.js";
...
export type RunnerDependencies = {
  ...
  getMarketData: (symbol: string, extensionId: string) => Promise<MarketSnapshot>;
};
...
export async function executeStrategy(
  ...
  try {
    ...
    if (strategy.maxDailyLossUsd != null && policyState.dailyPnlUsd <= -strategy.maxDailyLossUsd) {
    ...
    for (const symbol of strategy.symbols) {
      ...
      if (strategy.maxConcurrentPositions != null) {
        const openPositions = Object.keys(policyState.positionsByAsset ?? {}).length;
      ...
      const { results, aggregateStrength } = evaluateSignals(strategy.signals, market);
      ...
      if (sizeUsd <= 0 || market.currentPrice <= 0) {
        continue;
      }
      ...
      if (
        ...
        const offset = market.currentPrice * (strategy.entryRule.limitOffsetPercent / 100);
        limitPrice = side === "buy" ? market.currentPrice - offset : market.currentPrice + offset;
      }
      ...
      log.info(`order submitted for ${symbol}: ${result.outcome} (${result.orderId ?? "no-id"})`);
    }
    ...
    execution.completedAt = new Date().toISOString();
    log.error(`strategy execution failed: ${String(err)}`);
  }
