...
const priceCrossAbove: SignalEvaluator = (config, market) => {
  const threshold = Number(config.params.threshold ?? 0);
  const prev =
    market.previousClose ?? market.priceHistory[1] ?? market.currentPrice;
...
const rsiOverbought: SignalEvaluator = (config, market) => {
  ...
  return val >= threshold
    ? clamp01((val - threshold) / (100 - threshold))
    : 0;
};
...
const volatilityBreakout: SignalEvaluator = (config, market) => {
  ...
  return zScore >= multiplier
    ? clamp01((zScore - multiplier) / multiplier)
    : 0;
};
...
export function evaluateSignals(
  ...
  if (signals.length === 0) return { results: [], aggregateStrength: 0 };

  const results = signals.map((s) => evaluateSignal(s, market));
  const totalWeight = signals.reduce((s, sig) => s + sig.weight, 0);
  if (totalWeight === 0) return { results, aggregateStrength: 0 };

  const weightedSum = results.reduce(
    (sum, r, i) => sum + r.value * signals[i].weight,
    0,
  );
