...
const DYNAMIC_ARCHIVE_TEMPLATE_PRESETS = [
  {
    ...
    packageJson: {
      name: "@evil/..",
      version: "0.0.1",
      tigerpaw: { extensions: ["./dist/index.js"] },
...
function setupManifestInstallFixture(params: { manifestId: string }) {
  ...
  fs.writeFileSync(
    path.join(pluginDir, "tigerpaw.plugin.json"),
    JSON.stringify({
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "tigerpaw.plugin.json"),
    JSON.stringify({
  ...
  it("scans extension entry files in hidden directories", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        tigerpaw: { extensions: [".hidden/index.js"] },
      }),
    );
    fs.writeFileSync(
...
describe("installPluginFromDir", () => {
  ...
  it("uses tigerpaw.plugin.json id as install key when it differs from package name", async () => {
