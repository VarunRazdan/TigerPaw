...
export const backtestHandlers: GatewayRequestHandlers = {
    ...
    try {
      ...
      if (!strategyId) {
        respond(
          false,
          undefined,
          errorShape(ErrorCodes.INVALID_REQUEST, "strategyId is required"),
        );
        return;
      }
      ...
      validateTradingNumeric("initialCapitalUsd", initialCapitalUsd, 1, MAX_INITIAL_CAPITAL_USD);
      validateTradingNumeric("commissionPercent", commissionPercent, 0, MAX_COMMISSION_PERCENT);
      validateTradingNumeric("slippageBps", slippageBps, 0, MAX_SLIPPAGE_BPS);
      ...
      try {
        ...
        if (dataSource === "alpaca") {
          ...
          const { SyntheticDataProvider } = await import("../../trading/backtest/synthetic-provider.js");
  ...
  "backtest.generate": async ({ params, respond }) => {
    try {
      ...
      // Bounds validation
      validateTradingNumeric("days", days, 1, MAX_BACKTEST_DAYS);
      validateTradingNumeric("startPrice", startPrice, 0.01, MAX_START_PRICE);
