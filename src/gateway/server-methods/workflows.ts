...
import { checkOwnership, resolveOwnerIdentity } from "../ownership.js";
...
export const workflowsHandlers: GatewayRequestHandlers = {
  ...
  "workflows.save": async ({ params, client, respond }) => {
    ...
    try {
      ...
      if (existing) {
        ...
        const check = checkOwnership(existing.ownerId, owner?.ownerId);
        if (!check.allowed) {
          respond(
            false,
            undefined,
            errorShape(ErrorCodes.INVALID_REQUEST, check.reason ?? "ownership check failed"),
          );
          return;
        }
        ...
        if (owner) {
          workflow.ownerId = owner.ownerId;
          workflow.ownerLabel = owner.ownerLabel;
        }
      }
  ...
  "workflows.delete": async ({ params, client, respond }) => {
    const id = params.id as string | undefined;
    if (!id) {
      respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "id is required"));
      return;
    ...
    try {
      const owner = resolveOwnerIdentity(client);
      const existing = dalGetWorkflow(id);
      if (existing) {
        const check = checkOwnership(existing.ownerId, owner?.ownerId);
        if (!check.allowed) {
          respond(
  ...
  "workflows.toggle": async ({ params, client, respond }) => {
    const id = params.id as string | undefined;
    if (!id) {
      respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "id is required"));
      return;
  ...
  "workflows.import": async ({ params, respond }) => {
    ...
    try {
      ...
      const newId = `wf-${Date.now().toString(36)}-${randomBytes(3).toString("hex")}`;
      workflow.id = newId;
