 ...
 */
import { OwnershipError, resolveOwnerIdentity } from "../ownership.js";
import { ErrorCodes, errorShape } from "../protocol/index.js";
import type { GatewayRequestHandlers } from "./types.js";

function ownershipAwareError(err: unknown) {
...
export const strategiesHandlers: GatewayRequestHandlers = {
  ...
  "strategies.save": async ({ params, client, respond }) => {
    try {
      const owner = resolveOwnerIdentity(client);
      const { saveStrategy } = await import("../../trading/strategies/registry.js");
      const strategy = await saveStrategy(params, owner?.ownerId, owner?.ownerLabel);
      respond(true, { strategy }, undefined);
    } catch (err) {
      respond(false, undefined, ownershipAwareError(err));
    }
  },

  "strategies.delete": async ({ params, client, respond }) => {
    try {
      const id = params.id as string;
      if (!id) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "id is required"));
      ...
      const deleted = await deleteStrategy(id, owner?.ownerId);
  ...
  "strategies.toggle": async ({ params, client, respond }) => {
    try {
      const id = params.id as string;
      ...
      const strategy = await toggleStrategy(id, enabled, owner?.ownerId);
      if (!strategy) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "strategy not found"));
        return;
      }
