---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Tigerpaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Tigerpaw cron scheduling for periodic checks, or version status checks on a machine running Tigerpaw (laptop, workstation, Pi, VPS).
---

# Tigerpaw Host Hardening

## Overview

Assess and harden the host running Tigerpaw, then align it to a user-defined risk tolerance without breaking access. Use Tigerpaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Tigerpaw changes the host firewall, SSH, or OS updates; it does not.
...
5. Tigerpaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Tigerpaw can access sensitive data.
...
### 2) Run Tigerpaw security audits (read-only)

As part of the default read-only checks, run `tigerpaw security audit --deep`. Only offer alternatives if the user requests them:

1. `tigerpaw security audit` (faster, non-probing)
2. `tigerpaw security audit --json` (structured output)

Offer to apply Tigerpaw safe defaults (numbered):

1. `tigerpaw security audit --fix`

Be explicit that `--fix` only tightens Tigerpaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Tigerpaw version/update status (read-only)

As part of the default read-only checks, run `tigerpaw update status`.
...
- Credential hygiene notes (location of Tigerpaw creds, prefer disk encryption)
...
- Tigerpaw security audit (re-run)
...
After Tigerpaw install or first hardening pass, run at least one baseline audit and version check:

- `tigerpaw security audit`
- `tigerpaw security audit --deep`
- `tigerpaw update status`

Ongoing monitoring is recommended. Use the Tigerpaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `tigerpaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `tigerpaw update status`
...
Before creating, `tigerpaw cron list` and match on exact `name`. If found, `tigerpaw cron edit <id> ...`.
If not found, `tigerpaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `tigerpaw update status` (preferred for source checkouts and channels)
2. `npm view tigerpaw version` (published npm version)

## Tigerpaw command accuracy

Use only supported commands and flags:

- `tigerpaw security audit [--deep] [--fix] [--json]`
- `tigerpaw status` / `tigerpaw status --deep`
- `tigerpaw health --json`
- `tigerpaw update status`
- `tigerpaw cron add|list|runs|run`

Do not invent CLI flags or imply Tigerpaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Tigerpaw compaction:
